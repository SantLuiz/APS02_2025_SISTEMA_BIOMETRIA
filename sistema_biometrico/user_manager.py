...
class UserManager:
    ...
    def get_user_id(self, username: str) -> Optional[int]:
        ...
        """
        self.cursor.execute(
            "SELECT id FROM usuarios WHERE NOME = ?", (username.upper(),)
        )
    ...
    def get_fingerprints(self, user_id: int) -> List[np.ndarray]:
        ...
        self.cursor.execute(
            "SELECT template FROM DIGITAIS WHERE ID_USUARIO = ?", (user_id,)
        )
    ...
    def get_links_by_access(self, user_level: int) -> List:
        ...
        self.cursor.execute(
            "SELECT LINK FROM INFORMACOES WHERE CLASSIFICACAO <= ?", (user_level,)
        )
